...
export default class GameMath {
     ...
     */
    public static point_direction (x1:number,y1:number,x2:number,y2:number): number {
        const p1 = {
            x:x1,
            y:y1
        }
        ...
        return (GameMath.calculate_angle(p1.x,p1.y,p2.x,p2.y)*180)/Math.PI
    }
     ...
     */
    public static calculate_angle(x1:number,y1:number,x2:number,y2:number): number {
        const p1 = {
            x:x1,
            y:y1
        }
        ...
        if (p2.x > p1.x) {
            ...
            if (p2.y > p1.y) {
                // quad 2
                return GameMath.arctan(p1.x,p1.y,p2.x,p2.y)}
                // should be 1-90
            else {
                if (p2.y==p1.y) {
                    return 0}
                    ...
                    // quad 1
                    return 2*Math.PI+GameMath.arctan(p1.x,p1.y,p2.x,p2.y)
                    // 270-360
                }
            }
        }
            ...
            if (p2.x==p1.x) {
                ...
                if (p2.y == p1.y) {
                    ...
                    if (p2.y > p1.y) {
                        // quad 3
                        return Math.PI + GameMath.arctan(p1.x,p1.y,p2.x,p2.y)
                    }
                        // 90-180
                    else {
                        // quad 4
     ...
     * @returns The angle in radians.
     */
    public static arctan (x1:number,y1:number,x2:number,y2:number): number {
     ...
     */
    public static polar_move (x:number, y:number,angle:number,distance:number):any {
        return {
    ...
    /**
     * Rounds an angle to the nearest quadrant angle (0, 90, 180, 270).
     * @param angle The angle in degrees to round.
     * @returns The nearest quadrant angle (0, 90, 180, or 270).
     */
    public static roundToQuadrant(angle: number): number {
